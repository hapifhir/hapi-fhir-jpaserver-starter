...
class EmailUserServiceTest {
	...
	private static final List<String> REPORT_EMAILS = Arrays.asList("sushantk@apra.in", "skspider619@gmail.com");
	...
	void setUp() {
		...
		startDate = previousMonday.format(DateTimeFormatter.ISO_LOCAL_DATE); // "2025-04-21"
		endDate = previousSunday.format(DateTimeFormatter.ISO_LOCAL_DATE);   // "2025-04-27"
	...
	void sendWeeklyFacilitySummary_processDataForReportFails_logsErrorAndContinues() throws Exception {
		...
		when(helperService.processDataForReport(
			eq(startDate), eq(endDate), eq(ReportType.weekly), any(LinkedHashMap.class),
			eq(REPORT_ENV), eq("lab_test_clinic2"), eq(false)
		)).thenReturn(new ResponseEntity<>(createScoreCardItems("lab_test_clinic2", getIndicatorValuesForFacility("lab_test_clinic2", false)), HttpStatus.OK));
