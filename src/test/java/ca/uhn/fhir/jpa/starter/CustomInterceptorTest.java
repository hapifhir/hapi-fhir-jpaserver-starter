...
class CustomInterceptorTest {
	...
	// // we registered two custom interceptors via the property
	// 'hapi.fhir.custom-interceptor-classes'
	// // one is discovered as a Spring Bean, one instantiated via reflection
	// // both should be registered with the server and will add a custom extension
	// to any Patient resource created
	...
	// Patient readPat =
	// client.read().resource(Patient.class).withId(patId).execute();
