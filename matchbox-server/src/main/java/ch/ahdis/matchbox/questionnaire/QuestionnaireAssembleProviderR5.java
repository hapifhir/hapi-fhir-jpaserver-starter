 ...
 */
public class QuestionnaireAssembleProviderR5 {
	...
	public Questionnaire getQuestionnaire(String canonical) {
		...
		if (matchboxEngine !=null) {
			return (Questionnaire) matchboxEngine.getCanonicalResource(canonical, FhirVersionEnum.R5.getFhirVersionString());
		}
		return null;
	}

	public List<Questionnaire.QuestionnaireItemComponent> checkItemsList(Questionnaire questionnaire, List<Questionnaire.QuestionnaireItemComponent> items, String linkdIdPrefix, boolean assembleContext) {
		String subLinkdIdPrefix = linkdIdPrefix;
		if (items != null) {
			List<Questionnaire.QuestionnaireItemComponent> expanded = new ArrayList<Questionnaire.QuestionnaireItemComponent>();
			for (Questionnaire.QuestionnaireItemComponent item : items) {
				...
				if (extension != null) {
					...
					Questionnaire.QuestionnaireItemComponent copiedItem = item.copy();
