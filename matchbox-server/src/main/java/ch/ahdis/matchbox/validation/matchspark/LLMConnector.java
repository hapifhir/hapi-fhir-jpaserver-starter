...
/**
 * The OpenAIConnector class is used to create the http-POST-request to OpenAI. In order to create the request you need
 * to define a model and an API-key (which is stored as an environment variable). Four of the used prompts are displayed
 * here as final strings. Because there were 3 prototypes, a slightly different block of code for the respecting
 * methods. There could be some refactoring done here.
 */
public class LLMConnector {
	private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(LLMConnector.class);
	private static final String HUGGINGFACE_BASE_URL = "https://router.huggingface.co/v1";

	private final String LLM_PROVIDER;
	...
	public static LLMConnector getConnector(final CliContext cliContext) {
	 ...
	 */
	private LLMConnector(final CliContext cliContext) {
	...
	/**
	 * Final Prototype: Interprets the result with a LLM of OpenAI. Uses a function called createRequestBody(). Gets
	 * input from GUI.
	 *
	 * @param resource         The string received from the Matchbox containing the FHIR resource.
	 * @param operationOutcome The string received from the Matchbox containing the validation result
	 *                         (OperationOutcome).
	...
	public String interpretWithMatchbox(String resource, String operationOutcome) {
		...
		try {
			final String requestBody = createRequestBody(resource, operationOutcome);
	 ...
	 * @param operationOutcomeContent The ObjectNode received from the calling method containing the validation result
	 *                                (OperationOutcome).
