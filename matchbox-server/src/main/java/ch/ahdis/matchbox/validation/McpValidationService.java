...
public class McpValidationService {
	...
	public String mcpToolGetValidation(@ToolParam(description = "The FHIR resource to validate (JSON or XML)") final String resource,
		...
		try {
			...
			return "Error while initializing the validation engine: %s".formatted(e.getMessage());
		}
		if (engine == null) {
			return "Matchbox engine for profile '%s' could not be created, check the installed IGs".formatted(
				profile);
		}
		...
		try {
			...
			return "Error during validation: %s".formatted(e.getMessage());
		}

		// 3. Generate the textual representation of the validation report
