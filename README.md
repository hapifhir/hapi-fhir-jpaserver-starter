...
You can use the docker image generated by:

Build the docker imge:
...
```shell
$ docker run -it -e DB_URL="jdbc:postgresql://host.docker.internal:5432/db?currentSchema=hapi_fhir" -e DB_USERNAME="admin" -e DB_PASSWORD="admin" -e SPRING_PROFILES_ACTIVE="staging" fhir-server
```

`host.docker.internal` is a reference to the host computer running the docker container (likely your computer).

Update environment variables and their values accordingly.
