...
#RUN mvn -ntp -B -U -DskipTests package
#
...
WORKDIR /app
# Copy your already-built WAR
COPY --chown=nonroot:nonroot target/ROOT.war /app/main.war
# (Optional) copy your OTEL agent if you want it
# COPY --chown=nonroot:nonroot opentelemetry-javaagent.jar /app/

ENTRYPOINT ["java","-cp","/app/main.war","-Dloader.path=main.war!/WEB-INF/classes/,main.war!/WEB-INF/,/app/extra-classes","org.springframework.boot.loader.PropertiesLauncher"]